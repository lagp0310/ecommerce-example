  ...
  *
  ...
  (
    SELECT 
      ...
      customers.last_name,
      emails.email,
      ROUND((line_items.price * COALESCE(line_items.weight, line_items.quantity))::NUMERIC, 2) AS result_orders_total
    FROM
      carts,
      customers,
      emails,
    ...
    GROUP BY
      customers.first_name, customers.last_name, emails.email, orders.id, line_items.price, line_items.weight, line_items.quantity
    LIMIT rows_limit
  ) AS latest_orders;
  END;
